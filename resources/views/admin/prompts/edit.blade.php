...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.prompt.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.prompts.update", [$prompt->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="title">{{ trans('cruds.prompt.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $prompt->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('text') ? 'is-invalid' : '' }}" name="text" id="text">{{ old('text', $prompt->text) }}</textarea>
                @if($errors->has('text'))
                    <div class="invalid-feedback">
                        {{ $errors->first('text') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ai_platforms') ? 'is-invalid' : '' }}" name="ai_platforms[]" id="ai_platforms" multiple>
                    @foreach($ai_platforms as $id => $ai_platform)
                        <option value="{{ $id }}" {{ (in_array($id, old('ai_platforms', [])) || $prompt->ai_platforms->contains($id)) ? 'selected' : '' }}>{{ $ai_platform }}</option>
                    @endforeach
                </select>
